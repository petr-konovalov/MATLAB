...
function t = getOccupiedSegments(A, B, O, Q, R)
    ...
    for j = 1: size(Q, 1)
        ...
        [t1, t2] = getOccupiedSegment(pnts(1, :) - O, pnts(2, :) - O, u, v);
        if (t1 <= t2)
            tSz = tSz + 1;
            t(tSz, 1) = t1;
            t(tSz, 2) = t2;
...
function [t1, t2] = getOccupiedSegment(dir1, dir2, u, v)
    t1 = vectMult(dir1, u) / vectMult(dir1, v);
    t2 = vectMult(dir2, u) / vectMult(dir2, v);
